...
autoback 是一个面向 Linux 集群的增量文件备份系统，用于灾难恢复场景。
分为三个组件：`autoback-cli`（命令行工具）、`autoback-agent`（Client 端守护进程）、`autoback-server`（备份存储守护进程）。
...
```
autoback-cli        — 命令行交互工具
    ↓ gRPC (Unix socket, SO_PEERCRED)
autoback-agent      — 每台 Client 机器的后台守护进程
    ↓ gRPC (TCP, mTLS + SSH Key 签名)
autoback-server     — 备份存储 Server，管理 ZFS
```
...
| 组件 | 职责 | 对外接口 |
|------|------|---------|
| autoback-cli | 用户交互：规则管理、手动触发、进度查看、恢复请求 | 仅与本地 Agent 通过 Unix socket 通信 |
| autoback-agent | 定时调度、文件扫描、增量计算、批量同步、进度上报 | AgentService（给 CLI）+ BackupService 客户端（给 Server） |
| autoback-server | 数据接收落盘、ZFS dataset/quota/快照管理、恢复服务 | BackupService（TCP/mTLS） |
...
| 全局强制规则 + 用户 quota + 默认调度 + 允许主机 | Server: `/etc/autoback/server/config.yaml` | 管理员 |
| ZFS quota 强制执行 | ZFS dataset 自身 | ZFS |

### Server 配置

```yaml
# /etc/autoback/server/config.yaml
server:
  cert_file: "/etc/autoback/server/cert.pem"
  key_file: "/etc/autoback/server/key.pem"
  listen: "0.0.0.0:8443"
...
```yaml
# /etc/autoback/agent/config.yaml
agent:
  cert_file: "/etc/autoback/agent/cert.pem"
  key_file: "/etc/autoback/agent/key.pem"
...
hooks:
  on_task_failed: "/usr/local/bin/autoback-alert.sh"
  on_quota_warning: "/usr/local/bin/autoback-quota-warn.sh"
```
...
- `autoback rule add/remove/list/enable/disable` — 管理个人备份路径
- `autoback sync trigger/status` — 手动触发和进度查看
- `autoback quota` — 查看 quota 用量
- `autoback restore [--path]` — 请求恢复
...
```
autoback admin rule add/remove/list    ← 系统目录备份规则
autoback admin server add/remove/list  ← 目标 Server 管理
```
...
```
CLI → Agent: autoback restore [--path <target>]
Agent 校验:
...
```
Client 机器:
  /usr/bin/autoback                          ← CLI
  /usr/bin/autoback-agent                    ← Agent 守护进程
  /etc/autoback/agent/{config.yaml,cert.pem,key.pem}
  /etc/autoback/agent/user-rules/            ← 用户规则（每用户一个文件）
  /var/lib/autoback/agent/state.db           ← SQLite
  /etc/systemd/system/autoback-agent.service

Server 机器:
  /usr/bin/autoback-server                   ← Server 守护进程
  /etc/autoback/server/{config.yaml,cert.pem,key.pem}
  /etc/autoback/server/authorized_keys/      ← 用户公钥（按 hostname 分组）
  /etc/autoback/server/ca/                   ← CA 证书和私钥
  /var/lib/autoback/server/state.db          ← SQLite
  /etc/systemd/system/autoback-server.service
```
...
```
初始化:
  autoback-server init-ca                  → 自建 CA
  autoback-server init-cert                → Server 自签证书
  autoback-server sign-agent --hostname X  → 签发 Agent 证书

分发:
  管理员通过安全通道（scp）将证书分发到 Client 机器
  autoback admin cert install              → Agent 端安装
...
```
systemctl reload autoback-agent   → SIGHUP → 重载配置，不中断运行中任务
systemctl reload autoback-server  → SIGHUP → 重载配置，不中断现有连接
```
